...
def main():
    ...
    for id, text in enumerate(dataset['wb_descriptions']):
        print(f"{id}--------------------------------------------------------------")
        print(analyzes_pymorphy({"VERB", "impr"}, text))
        print(analyzes_spacy("VERB", "Mood", "Imp", text))
        print(analyzes_stanza("VERB", "Mood=Imp", text))
        print(analyzes_natasha("VERB", "Mood", "Imp", text))
